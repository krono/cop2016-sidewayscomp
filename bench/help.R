...
summarize.bench <- function(bench, rigorous, multi.variate, group.by, reference.vm) {
  if (rigorous) {
    bench.summary <- ddply(bench, as.quoted(group.by), plyr::summarize, 
                           overall=FALSE,
                           mean=mean(value),
                           median=median(value),
                           stdev=sd(value),
    ...
    if (multi.variate) {
      ...
      for (var.val in levels(factor(bench.summary$variable_values))) {
        ...
        .err <- bootstrapTo(.tot, group.by[[1]], group.by[[2]], reference.vm, 'value')
        .norm <- normalizeTo(.sum, group.by[[1]], group.by[[2]], reference.vm, 'mean')
        .merge <- merge(.norm, .err)
        df <- rbind(df, .merge)
      }
      bench.summary <- df
    } else {
      bench.err <- bootstrapTo(bench, group.by[[1]], group.by[[2]], reference.vm, 'value')
      bench.summary <- merge(
        normalizeTo(bench.summary, group.by[[1]], group.by[[2]], reference.vm, 'mean'),
        bench.err
      )
    }
  } else {
    ...
    if (multi.variate) {
      ...
      for (var.val in levels(factor(bench.summary$variable_values))) {
        ...
        df <- rbind(df, normalizeTo(b, group.by[[1]], group.by[[2]], reference.vm, 'mean'))
      }
      bench.summary <- df
    } else {
      bench.summary <- normalizeTo(bench.summary,  group.by[[1]], group.by[[2]], reference.vm, 'mean')
    }
  }
  bench.summary
}
